...
| PostgreSQL hybrid search | `websearch_to_tsquery` without prefix matching |

The most visible effect is prefix matching. A prefix that works in PostgreSQL
FTS mode may not match the same documents in PostgreSQL hybrid mode.
