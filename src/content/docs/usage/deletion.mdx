...
Remote deletion execution is gated; set `MSGVAULT_ENABLE_REMOTE_DELETE=1` to opt in. Read-only modes (`--list` and `--dry-run`) work without the gate. Gmail deletion permanently removes messages through the Gmail batch API. For IMAP accounts, msgvault uses `UID STORE \Deleted` followed by `UID EXPUNGE` to permanently remove messages. On large batches it falls back to per-message deletion. Dry-run mode works for both Gmail and IMAP accounts.
