---
title: Deduplication
description: Find and merge duplicate messages across accounts and collections with a reversible, four-rung safety ladder that never deletes anything by default.
---
...
Every dedup-related command sits on one of five rungs. You climb them deliberately, one explicit action at a time. msgvault never escalates from one rung to the next on its own: applying dedup never implies a local hard delete, and a local hard delete never implies a remote delete.
...
|---|---|---|---|
| 00 | Backup (automatic) | runs before rungs 02 and 03 | produces a recoverable backup file |
...
Batch IDs look like `dedup-20260503-091500-7-personal-0d4cb6f1`. Note the one printed by your run; the later steps take it.
