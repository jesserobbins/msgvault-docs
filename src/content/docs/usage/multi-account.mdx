...
<img src="/concepts/account-collection-concept.png" alt="Accounts on the left are individual ingest sources, each carrying the identifiers that mean you inside that source. Collections on the right are named groups of accounts: All contains every account, with Personal and Work as deliberate subsets." style="width: 100%; max-width: 760px; margin: 1.5rem auto; display: block;" />
...
Each confirmed identifier records the signals that confirmed it, such as the account identifier itself, an OAuth address, a sent-folder or sent-label hit, or a `manual` entry you added. An identifier accumulates signals over time as new evidence appears; it is removed only by `identity remove`.
