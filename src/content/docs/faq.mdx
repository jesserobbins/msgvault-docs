...
Only if you explicitly run the full deletion workflow. Staging messages for deletion in the TUI does not touch Gmail or your IMAP provider. You must run `MSGVAULT_ENABLE_REMOTE_DELETE=1 msgvault delete-staged` to execute staged deletions. Deletion is permanent on both Gmail and IMAP, and your local archive is always preserved. See [Deleting Email](/usage/deletion/) for the complete process.
