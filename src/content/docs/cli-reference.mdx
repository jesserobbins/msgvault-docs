...
Execute staged remote deletions. Gmail deletion permanently removes messages through the Gmail batch API. IMAP deletion removes messages from the provider using IMAP delete/expunge behavior.
...
Execution requires `MSGVAULT_ENABLE_REMOTE_DELETE=1`; `--list` and `--dry-run` work without the gate. Deletion is permanent on both Gmail and IMAP, so review staged batches before executing.
